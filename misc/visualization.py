...
def check_video_rotation(filename):
    """
    Check video rotation metadata and return appropriate OpenCV rotation code.
    
    Args:
        filename: Path to the video file
        
    Returns:
        OpenCV rotation code or None if no rotation needed
    """
    # thanks to
    ...
    try:
        ...
        # Check for rotation in different possible locations
        rotation_value = None
        
        # Try to find rotation in streams
        for stream in meta_dict.get('streams', []):
            if stream.get('codec_type') == 'video':
                # Check tags for rotation
                tags = stream.get('tags', {})
                if 'rotate' in tags:
                    rotation_value = int(tags['rotate'])
                    break
                # Some videos store rotation as 'rotation'
                elif 'rotation' in tags:
                    rotation_value = int(tags['rotation'])
                    break
                # Check side_data for displaymatrix rotation
                    ...
                    for side_data in stream['side_data_list']:
                        if side_data.get('side_data_type') == 'Display Matrix':
                            ...
                            rotation_value = int(float(rotation_str))
                            break
        
        # Convert rotation value to OpenCV rotation code
        if rotation_value is not None:
            ...
            # For other values, no rotation applied
                
    except (KeyError, ValueError, TypeError) as e:
        # If any error occurs, just return None (no rotation)
        print(f"Warning: Could not determine video rotation for {filename}: {e}")
        pass
    except Exception as e:
        # Catch any other ffmpeg-related errors
        print(f"Warning: Error probing video file {filename}: {e}")
        pass
